...
class DataLoader:
    ...
    def __init__(self, filePath, batchSize, imgSize, maxTextLen):
        ...
        f = open(filePath + 'words.txt',  "r", encoding='utf-8')
        chars = set()
        bad_samples = []
        bad_samples_reference = ['5.bmp', '1233184155799.bmp']
        for line in f:
            # ignore comment line
            ...
            Text = self.truncateLabel(' '.join(lineSplit[8:]), maxTextLen)
            gtText = Text.strip()
            print(gtText)
            chars = chars.union(set(list(gtText)))
            #print(chars)
            #print(gtText)  ## taking the unique characters present

            # check if image is not empty
            if not os.path.getsize(fileName):
                bad_samples.append(lineSplit[0] + '.bmp')
                continue
