...
class FilePaths:
    ...
    fnCharList = '../src/data/charList.txt'
    fnAccuracy = '../src/data/accuracy.txt'
    fnTrain = '../src/data/'
    fnInfer = '../src/data/1.bmp'
    

    # /content/drive/MyDrive/Handwritten_Text_Recognition/data/test.png

    fnCorpus = '../src/data/corpus.txt'
...
def infer(model, fnImg):
    ...
    #print("Recognized Text With Correction", correct_sentence(recognized[0]))
...
def main():
    ...
    if args.train or args.validate:
        ...
        open(FilePaths.fnCharList, 'w', encoding='UTF8').write(str().join(loader.charList))

        # save words contained in dataset into file
        open(FilePaths.fnCorpus, 'w', encoding='UTF8').write(str(' ').join(loader.trainWords + loader.validationWords))
    ...
    # infer text on test image
        ...
        time_e = end - start
        print("Recognized in", time_e)

def infer_by_web(path, option):
    decoderType = DecoderType.BestPath
    print(open(FilePaths.fnAccuracy).read())
    #model = Model(open(FilePaths.fnCharList).read(),
                #  decoderType, mustRestore=False, encoding='utf-8')
    start = time.time()
